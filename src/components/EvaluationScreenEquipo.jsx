...
const EvaluationScreenEquipo = ({ onBack, onComplete, onSkipToResults, username }) => {
  ...
  const saveSubsectionProgress = async (sectionData, subsectionData, subsectionAnswers) => {
    try {
      ...
      const key = `${sectionData.id}-${subsectionData.id}`;
      setCompletedSubsections(prev => new Set([...prev, key]));
  ...
  const saveSectionProgress = async (sectionData, allSubsectionResults) => {
    try {
      const user = apiService.getCurrentUser();
      if (!user) return;
              ...
              <div className="p-6">
                <div className="space-y-4">
                  {currentSubsectionData?.preguntas?.map((question, index) => {
                    ...
                    const isCompleted = isSubsectionCompleted(currentSectionData.id, currentSubsectionData.id);
                    ...
                    return (
                      <div key={index} className="border-b border-gray-200 pb-4 last:border-b-0">
                        ...
                        <div className={`flex space-x-4 ${isCompleted ? 'opacity-60 pointer-events-none' : ''}`}>
                          {[
                            ...
                            <button
                              ...
                              className={`px-6 py-2 rounded-lg text-white font-medium transition-all duration-200 ${
                                selectedAnswer === option.value 
                                  ...
                                  : option.color + (isCompleted ? ' cursor-not-allowed' : '')
                              }`}
                            >
                              {option.label}
                            </button>
                        ...
                        {isCompleted && (
                          <div className="mt-2 text-sm text-green-600 flex items-center">
                            ...
                            Subsección completada anteriormente
                          </div>
                        )}
                      </div>
                    );
                ...
                <div className="mt-8 flex justify-center">
                  {isSubsectionCompleted(currentSectionData.id, currentSubsectionData.id) ? (
                    ...
                    >
                      <CheckCircle className="w-4 h-4" />
                      <span>Continuar (Ya completada)</span>
                    </Button>
                  ) : (
                    <Button
                      onClick={handleNextSubsection}
                    ...
                    >
                      ...
                      <span>
                        {isLastSubsection ? 'Completar Sección' : 'Siguiente Subsección'}
                      </span>
                    </Button>
                  )}
                </div>
